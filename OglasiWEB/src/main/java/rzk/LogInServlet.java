...
public class LogInServlet extends HttpServlet {
	...
	private static String getLookupName() {
		...
		final String interfaceName = UserBeanRemote.class.getName();
		// let's do the lookup
	...
	private static UserBeanRemote doLookup() {
		Context context = null;
		UserBeanRemote bean = null;
		try {
			context = getInitialContext();
			String lookupName = getLookupName();
			System.out.println("JNDI ime:   " + lookupName);
			bean = (UserBeanRemote) context.lookup(lookupName);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//bean.ispisi();
		username = bean.login(username, password);
		
		if(username.equals(""))
			request.getRequestDispatcher("/greska.jsp").forward(request, response);
		else {
